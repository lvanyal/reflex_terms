---
layout: default
title: Privacy Policy
---

# Privacy Policy

**Chess Reflex: ELO AI Puzzles**

*Effective Date: November 2, 2025*

*Last Updated: November 2, 2025*

## 1. Introduction

Welcome to Chess Reflex: ELO AI Puzzles ("we," "our," or "the App"). We respect your privacy and are committed to protecting your personal information. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our mobile application.

By using the App, you consent to the data practices described in this Privacy Policy. If you do not agree with this Privacy Policy, please do not use the App.

## 2. Information We Collect

### 2.1 Information Collected Automatically

When you use our App, we automatically collect certain information:

**Device Information:**
- Device type and model
- Operating system and version
- Unique device identifiers
- Device language and region settings
- Time zone

**Usage Information:**
- App features accessed
- Puzzles completed and performance data
- ELO rating and progress
- Session duration and frequency
- In-app purchases and subscription status
- App crashes and error reports

**Analytics Information:**
- App performance metrics
- User interaction patterns
- Feature engagement statistics
- Technical diagnostics

### 2.2 Information You Provide

We collect information you voluntarily provide:
- Email address (if provided for support)
- Feedback and correspondence
- User preferences and settings

### 2.3 Information We Do NOT Collect

We do not collect:
- Personal names or usernames
- Location data
- Contact lists
- Photos or media files
- Social media information
- Health or biometric data

## 3. How We Use Your Information

We use the collected information to:

### 3.1 Provide and Maintain the App
- Deliver chess puzzles and track your progress
- Calculate and update ELO ratings
- Save your game state and preferences
- Provide customer support

### 3.2 Improve the App
- Analyze usage patterns to improve features
- Fix bugs and technical issues
- Develop new puzzles and features
- Optimize app performance

### 3.3 Manage Subscriptions
- Process premium subscriptions
- Provide access to premium features
- Send subscription-related notifications

### 3.4 Analytics and Performance
- Monitor app stability and performance
- Understand user engagement
- Conduct A/B testing for feature optimization
- Generate aggregated statistics

## 4. Third-Party Services

We use the following third-party services that may collect information:

### 4.1 Firebase Analytics (Google)
**Purpose:** App analytics and performance monitoring

**Data Collected:**
- App usage statistics
- Device information
- Performance metrics
- Crash reports

**Privacy Policy:** https://firebase.google.com/support/privacy

### 4.2 Adapty
**Purpose:** Subscription management and paywall optimization

**Data Collected:**
- Device identifiers
- Purchase history
- Subscription status
- A/B testing data

**Privacy Policy:** https://adapty.io/privacy

### 4.3 Apple App Store
**Purpose:** App distribution and payment processing

**Data Collected:**
- Purchase information
- App Store account details (not shared with us)

**Privacy Policy:** https://www.apple.com/legal/privacy

## 5. Data Storage and Security

### 5.1 Storage Location
- Data is stored on secure servers provided by our third-party services
- Firebase data is stored on Google Cloud Platform servers
- We do not store personal data on our own servers

### 5.2 Data Retention
- Analytics data: Retained for 14 months
- Crash reports: Retained for 90 days
- Subscription data: Retained while subscription is active plus 6 months
- You can request deletion of your data at any time

### 5.3 Security Measures
We implement appropriate security measures including:
- Encrypted data transmission (HTTPS/TLS)
- Secure authentication for services
- Regular security updates
- Limited access to data

However, no method of electronic transmission or storage is 100% secure, and we cannot guarantee absolute security.

## 6. Data Sharing and Disclosure

### 6.1 We Do NOT:
- Sell your personal information
- Share your data with advertisers
- Use your data for marketing outside the App
- Transfer data to third parties except as described in this policy

### 6.2 We May Share Data:
- With service providers necessary for app operation (Firebase, Adapty)
- If required by law or legal process
- To protect our rights or safety
- With your explicit consent
- In aggregated, anonymized form for analytics

## 7. Children's Privacy

### 7.1 Age Requirements
- The App is not intended for children under 13
- We do not knowingly collect information from children under 13
- If we discover we have collected information from a child under 13, we will delete it immediately

### 7.2 Parental Rights
Parents who believe we have inadvertently collected information from their child should contact us immediately at vphtst@gmail.com

## 8. Your Rights and Controls

### 8.1 Your Rights
You have the right to:
- Access your personal information
- Request correction of your data
- Request deletion of your data
- Opt-out of analytics collection
- Withdraw consent at any time

### 8.2 How to Exercise Your Rights

**Opt-out of Analytics:**
- iOS: Settings > Privacy > Analytics & Improvements > Toggle off
- Within App: Settings > Privacy > Analytics (if available)

**Request Data Access/Deletion:**
Email us at vphtst@gmail.com with your request and device identifier

**Manage Subscriptions:**
- iOS: Settings > [Your Name] > Subscriptions

**Limit Ad Tracking:**
- iOS: Settings > Privacy > Tracking > Toggle off "Allow Apps to Request to Track"

## 9. International Data Transfers

Your information may be transferred to and processed in countries other than your own. These countries may have different data protection laws. By using the App, you consent to such transfers.

We ensure appropriate safeguards are in place for international transfers in compliance with applicable laws.

## 10. California Privacy Rights (CCPA)

California residents have additional rights under the California Consumer Privacy Act:

- Right to know what personal information is collected
- Right to know if personal information is sold or disclosed (we do NOT sell data)
- Right to refuse the sale of personal information
- Right to access personal information
- Right to request deletion
- Right to equal service and non-discrimination

To exercise these rights, contact us at vphtst@gmail.com

## 11. European Union Rights (GDPR)

EU residents have additional rights under the General Data Protection Regulation:

**Legal Basis for Processing:**
- Consent: For analytics and optional features
- Legitimate interests: For app functionality and improvements
- Contract: For subscription services

**Additional Rights:**
- Right to data portability
- Right to restrict processing
- Right to object to processing
- Right to lodge a complaint with supervisory authorities

**Data Protection Contact:**
Email: vphtst@gmail.com

## 12. Changes to This Privacy Policy

We may update this Privacy Policy from time to time. Changes will be posted in the App with an updated "Last Updated" date. Material changes will be notified through the App or via email if available.

Your continued use of the App after changes constitutes acceptance of the updated Privacy Policy.

## 13. Data Breach Notification

In the event of a data breach that may affect your personal information, we will:
- Notify affected users within 72 hours of discovery
- Provide information about the breach and potential impacts
- Offer guidance on protective measures
- Cooperate with relevant authorities

## 14. Contact Information

For questions, concerns, or requests regarding this Privacy Policy or your personal information:

**Email:** vphtst@gmail.com

**App Name:** Chess Reflex: ELO AI Puzzles

**Response Time:** We aim to respond to all privacy inquiries within 30 days

## 15. Additional Information

### 15.1 Do Not Track Signals
The App does not respond to Do Not Track signals as there is no consistent standard for compliance.

### 15.2 Third-Party Links
The App may contain links to third-party websites. We are not responsible for the privacy practices of these sites.

### 15.3 Anonymized Data
We may use anonymized and aggregated data for research, analytics, and improving chess education. This data cannot be used to identify you.

### 15.4 Open Source Libraries
The App may use open source libraries that have their own privacy practices. These do not collect personal information unless specified.

---

*By using Chess Reflex: ELO AI Puzzles, you acknowledge that you have read, understood, and agree to this Privacy Policy.*